...
my_debchange() {
    ...
    echo "python-fbrehm-libs (${version}) ${stage}; urgency=low" >"${TMP_FILE}"
    echo >>"${TMP_FILE}"
    for entry in "$@"; do
        if [ "${entry}" != "" ] ; then
            echo "  * ${entry}" >>"${TMP_FILE}"
    ...
    echo " -- ${DEBFULLNAME:-Developer} <${DEBEMAIL:-frank@brehm-online.com}>  $(date -R)" >>"${TMP_FILE}"
    echo >>"${TMP_FILE}"
...
scripts="src/fbrehm/__init__.py"
